  ...
  return (
    <div className="max-w-4xl space-y-6">
      <div>
        ...
        <p className="text-gray-500">Esta información se refleja en Equipo y Chat.</p>
      </div>
      ...
      <form onSubmit={submit} className="bg-white rounded-xl shadow-sm border p-6 space-y-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">Color y foto usados en chat, equipo y sidebar.</p>
          </div>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <Field label="Nombre" value={form.name} onChange={(value) => setForm({ ...form, name: value })} required />
          <Field label="URL foto de perfil" value={form.avatar} onChange={(value) => setForm({ ...form, avatar: value })} placeholder="https://..." />
        </div>

        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">Color de usuario</label>
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Quién es / bio</label>
          <textarea value={form.bio} onChange={(event) => setForm({ ...form, bio: event.target.value })} className="w-full px-3 py-2 border rounded-lg" rows={4} placeholder="Ej: Cofundador, backend, DevOps, ventas..." />
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <Field label="Web personal" value={form.websiteUrl} onChange={(value) => setForm({ ...form, websiteUrl: value })} placeholder="https://..." />
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          {saved && <span className="text-sm text-green-600">Perfil actualizado</span>}
        </div>
      </form>
    </div>
  );
